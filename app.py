    ...
    <style>
        ...
        *{box-sizing:border-box}
        ...
        .axion-mark{
            ...
            height:27px;
            border-radius:8px;
            background:linear-gradient(135deg,#536fca,#7a8fe5);
            color:#ffffff;
            font-size:.78rem;
            font-weight:900;
            box-shadow:0 3px 10px rgba(83,111,202,.22);
        }
...
top = st.columns([4.8, .72, .72, .72, .72, .72], gap="small")

with top[0]:
    st.markdown(
        """
        <div class="axion-brand">
            <button class="axion-brand-button" disabled>
                <span class="axion-mark">◈</span>
                <span class="axion-brand-name">AXION</span>
                <span class="axion-brand-separator">|</span>
                <span class="axion-brand-section">Monitoramento Industrial</span>
            </button>
        </div>
        """,
        unsafe_allow_html=True,
...
if st.session_state.get("show_account_menu", False):
    ...
    with account_user:
        st.markdown(
            f"""
            <div class="axion-account-menu">
                <span class="axion-account-name">{current_user_name}</span>
                <span class="axion-account-role">{current_user_role}</span>
            </div>
            """,
            unsafe_allow_html=True,
        )
